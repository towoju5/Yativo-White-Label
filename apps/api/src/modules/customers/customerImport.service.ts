  ...
  try {
    await prisma.customer.create({
      data: {
        ...
        status,
        // KYC status isn't seeded from Yativo — it no longer exposes a general customer-level
        // status (endorsements are now per-service and checked separately); imported customers
        // start at the schema default (NOT_STARTED) like any other new signup.
