                      ...
                      .map(([key, value]) => (
                        <div key={key} className="flex items-center justify-between py-2 text-sm">
                          ...
                          <dd className="flex items-center gap-2 font-mono">
                            {value}
                            <button onClick={() => copy(value)} className="text-muted-foreground hover:text-foreground" aria-label={`Copy ${key}`}>
      ...
      <Card>
        ...
        <CardContent>
          {currenciesQuery.isLoading ? (
            ...
            <p className="rounded-lg border border-dashed border-border p-6 text-center text-sm text-muted-foreground">
              {accounts.length > 0 ? "You already have an account for every available currency." : "No virtual account currencies are available yet."}
            </p>
          ) : (
            <div className="grid gap-3 sm:grid-cols-2">
              {availableCurrencies.map((c) => {
                ...
                return (
                  <div key={c.currency} className={cn("flex flex-col gap-2 rounded-xl border border-border p-4", !c.eligible && "opacity-80")}>
                    ...
                    {c.eligible ? (
                      <Button size="sm" disabled={pending} onClick={() => { setCreatingCurrency(c.currency); createMutation.mutate(c.currency); }}>
                        {pending ? "Creating…" : "Create account"}
                      </Button>
                    ) : c.hostedKycUrl ? (
                      <Button size="sm" variant="outline" asChild>
                        <a href={c.hostedKycUrl} target="_blank" rel="noreferrer">
                          Start verification <ExternalLink className="h-3.5 w-3.5" />
                        </a>
                      </Button>
                    ) : (
