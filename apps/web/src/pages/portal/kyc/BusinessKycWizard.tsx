...
const STEPS = ["Business info", "Addresses", "Owners & directors", "Owner ID documents", "Risk & purpose", "Business document", "Review"];
...
export default function BusinessKycWizard({ countries, countriesLoading }: { countries: KycCountry[]; countriesLoading?: boolean }) {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => {
      toast({ title: "Verification submitted", description: "We'll review your business details shortly." });
  ...
  return (
    ...
    <WizardShell
      ...
      subtitle="Business KYB — details on the company and its owners."
      steps={STEPS}
      current={step}
      onBack={goBack}
      onNext={goNext}
    ...
    >
      ...
      {step === 0 && (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            <Label>Incorporation country (optional)</Label>
          ...
          <div className="space-y-1.5">
            ...
            <Input {...form.register("primaryWebsite")} placeholder="https://" />
          </div>
          <div className="grid grid-cols-[5rem_1fr] gap-2">
            <div className="space-y-1.5">
              <Label>Dial code</Label>
              <Input {...form.register("phoneCallingCode")} placeholder="+1" />
            </div>
            <div className="space-y-1.5">
              <Label>Phone</Label>
          ...
          {(errors.businessLegalName || errors.businessTradeName || errors.registrationNumber || errors.businessType) && (
            <p className="text-xs text-destructive sm:col-span-2">Fill in the required fields above.</p>
          )}
        </div>
      )}
      ...
      {step === 1 && (
        <div className="space-y-6">
          <div>
            <h3 className="mb-2 text-sm font-semibold">Registered address</h3>
          ...
          <div>
            ...
            <AddressFields
              ...
              proofHint="Lease, utility bill, or similar — required if different from the registered address."
            />
          </div>
        </div>
      )}
          ...
          {personFields.map((person, i) => (
            <div key={person.id} className="space-y-4 rounded-xl border border-border p-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold">Owner / director {i + 1}</h3>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <div className="space-y-1.5">
                  <Label>Title</Label>
                  <Input {...form.register(`associatedPersons.${i}.title`)} placeholder="CEO, Managing Member…" />
                </div>
                <div className="space-y-1.5">
                  <Label>Email</Label>
              ...
              <div className="grid gap-2 sm:grid-cols-2">
                {(
                  [
                    ["hasOwnership", "Has ownership"],
                    ["hasControl", "Has control"],
                    ["isSigner", "Authorized signer"],
                    ["isDirector", "Director"],
                  ] as const
                ).map(([field, label]) => (
              ...
              <div>
                <h4 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">Residential address</h4>
      ...
      {step === 3 && (
        <div className="space-y-6">
          ...
          {personFields.map((person, i) => {
            ...
            return (
              <div key={person.id} className="space-y-4 rounded-xl border border-border p-4">
                <h3 className="text-sm font-semibold">{personName || `Owner / director ${i + 1}`}</h3>
                <div>
                  <h4 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">1. Tax ID</h4>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5 sm:col-span-2">
                      <Label>SSN / EIN / ITIN</Label>
                      <Input {...form.register(`associatedPersons.${i}.identifyingInformation.0.number`)} />
                    </div>
                  </div>
                </div>
                <div>
                  <h4 className="mb-2 text-xs font-semibold uppercase text-muted-foreground">2. Government photo ID</h4>
                  <BusinessPhotoIdFields
                    form={form}
          ...
          {errors.associatedPersons && <p className="text-xs text-destructive">Both identity documents are required for every owner/director.</p>}
        </div>
      )}
      ...
      {step === 4 && (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-2 sm:col-span-2">
            <Label>High-risk activities (select any that apply)</Label>
            ...
            {highRisk.length > 0 && (
              <div className="space-y-1.5 pt-2">
                <Label>Explain the flagged activity</Label>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3 sm:col-span-2">
            <div>
              <p className="text-sm font-medium">Conducts money services</p>
              <p className="text-xs text-muted-foreground">Remittance, currency exchange, or similar regulated MSB activity.</p>
            </div>
          ...
          {conductsMsb && (
            <>
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Describe the money services activity</Label>
                <Textarea {...form.register("conductsMoneyServicesDescription")} rows={2} />
              </div>
              <div className="space-y-1.5 sm:col-span-2">
                <Label>Compliance screening program</Label>
          ...
          <div className="flex items-center justify-between rounded-lg border border-border p-3 sm:col-span-2">
            <div>
              <p className="text-sm font-medium">Politically exposed person (PEP)</p>
              <p className="text-xs text-muted-foreground">Any owner/director is a PEP or close associate of one.</p>
            </div>
            <Switch checked={form.watch("pepStatus")} onCheckedChange={(v) => form.setValue("pepStatus", v, { shouldValidate: true })} />
          </div>
          <div className="flex items-center justify-between rounded-lg border border-border p-3 sm:col-span-2">
            <p className="text-sm font-medium">Processes payments for third-party MSBs</p>
          ...
          <div className="space-y-1.5">
            <Label>Operates in prohibited countries (optional)</Label>
          ...
          <div className="space-y-1.5">
            <Label>Estimated annual revenue, USD (optional)</Label>
            <Input {...form.register("estimatedAnnualRevenueUsd")} placeholder="e.g. 1000000_to_5000000" />
          </div>
      ...
      {step === 5 && (
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <Input {...form.register("documents.0.description")} placeholder="Certificate of Incorporation" />
          </div>
          <div className="sm:col-span-2">
            <FileField form={form} name="documents.0.file" label="File" />
          </div>
      ...
      {step === 6 && (
        <div className="space-y-4">
          ...
          <dl className="divide-y divide-border text-sm">
            {[
              ["Business", values.businessLegalName],
              ["Trade name", values.businessTradeName],
              ["Type", values.businessType ? humanize(values.businessType) : ""],
              ["Registered address", `${values.registeredAddress?.streetLine1}, ${values.registeredAddress?.city}, ${values.registeredAddress?.country}`],
              [
                "Owners / directors",
                (values.associatedPersons ?? []).map((p) => `${p.firstName} ${p.lastName} (${p.ownershipPercentage}%)`).join(", "),
              ],
              ["Account purpose", accountPurpose ? humanize(accountPurpose) : ""],
              ["High-risk activities", highRisk.length ? highRisk.map(humanize).join(", ") : "None"],
            ].map(([label, val]) => (
