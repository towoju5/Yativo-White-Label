...
export function AuroraLandingPage() {
  ...
  const features = [
    { icon: Wallet, title: t("landing.aurora.feature1Title", "One beautiful balance"), desc: t("landing.aurora.feature1Desc", "Every wallet, every currency, always reconciled.") },
    { icon: Send, title: t("landing.aurora.feature2Title", "Send in seconds"), desc: t("landing.aurora.feature2Desc", "Live quotes, instant confirmation, zero paperwork.") },
    { icon: CreditCard, title: t("landing.aurora.feature3Title", "Cards on demand"), desc: t("landing.aurora.feature3Desc", "Issue a virtual card the moment you're verified.") },
    { icon: ShieldCheck, title: t("landing.aurora.feature4Title", "Bank-grade ledger"), desc: t("landing.aurora.feature4Desc", "Every transaction, reconciled and auditable.") },
  ];
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <Link to="/portal/login" className="text-sm font-medium text-muted-foreground hover:text-foreground">
              {t("landing.aurora.logIn", "Log in")}
            </Link>
            <Button asChild size="sm" className="rounded-full">
              <Link to="/portal/signup">{t("landing.aurora.signUpFree", "Sign up free")}</Link>
            </Button>
          </div>
        </div>
      </header>
        ...
        <div className="relative mx-auto max-w-7xl px-6 py-16 lg:py-24">
          <div className="mx-auto max-w-2xl text-center">
            <span className="mb-4 inline-flex items-center gap-1.5 rounded-full border border-border bg-card px-3 py-1 text-xs font-semibold text-primary shadow-soft">
              <Sparkles className="h-3 w-3" /> {t("landing.aurora.badge", "Beautifully simple money")}
            </span>
            <h1 className="font-heading text-4xl font-bold leading-[1.08] tracking-tight sm:text-5xl">
              {t("landing.aurora.heroHeadlinePrefix", "Every account,")}{" "}
              <span className="bg-gradient-to-r from-primary to-secondary bg-clip-text text-transparent">{t("landing.aurora.heroHeadlineHighlight", "one gorgeous view.")}</span>
            </h1>
            <p className="mx-auto mt-5 max-w-md text-lg text-muted-foreground">
              {t("landing.aurora.heroSubheadline", "{{productName}} brings wallets, payouts, and cards together in a dashboard people actually enjoy opening.", { productName })}
            </p>
          ...
          <div className="mx-auto mt-16 grid max-w-4xl gap-5 lg:grid-cols-3">
            <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-primary/10 via-card to-secondary/10 p-6 shadow-elevated lg:col-span-2">
              <p className="text-xs font-semibold uppercase tracking-wider text-primary">{t("landing.aurora.totalBalance", "Total balance")}</p>
              <p className="mt-1 font-heading text-3xl font-bold">{t("landing.aurora.demoBalance", "$8,942.10")}</p>
              <div className="mt-6 grid grid-cols-3 gap-3">
                {[
                  { icon: Send, label: t("landing.aurora.demoActionSend", "Send") },
                  { icon: Wallet, label: t("landing.aurora.demoActionDeposit", "Deposit") },
                  { icon: CreditCard, label: t("landing.aurora.demoActionCard", "Card") },
                ].map((a) => (
            ...
            <div className="rounded-3xl border border-border bg-card p-6 shadow-elevated">
              <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">{t("landing.aurora.thisMonth", "This month")}</p>
              <p className="mt-2 font-heading text-2xl font-bold">{t("landing.aurora.demoGrowth", "+12.4%")}</p>
              <p className="mt-1 text-xs text-muted-foreground">{t("landing.aurora.vsLastMonth", "vs. last month")}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="bg-card/40 py-20">
        <div className="mx-auto max-w-7xl px-6">
          <div className="mb-12 max-w-xl">
            <h2 className="font-heading text-3xl font-bold tracking-tight">{t("landing.aurora.featuresHeading", "Everything your money needs")}</h2>
            <p className="mt-3 text-muted-foreground">{t("landing.aurora.featuresSubheading", "No hidden fees, no confusing menus — just banking that works.")}</p>
          </div>
